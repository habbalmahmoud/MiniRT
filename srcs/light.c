...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/16 09:55:08 by mhabbal           #+#    #+#             */
/*   Updated: 2025/04/16 09:55:08 by mhabbal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/miniRT.h"

void	in_shd(t_cor *scene, t_ish_utils *shadow_utils, float normal[3],
			float hit_point[3])
{
	int	i;
	...
	while (++i < 3)
		shadow_utils->shadow_orig[i] = hit_point[i] + normal[i]
			* shadow_utils->epsilon;
	subtract(scene->light.cor, shadow_utils->shadow_orig,
		shadow_utils->light_dir);
	shadow_utils->light_distance = sqrt(dot(shadow_utils->light_dir,
				shadow_utils->light_dir));
...
int	in_shd2(t_cor *scene, t_ish_utils *shadow_utils)
{
	...
	{
		shadow_utils->t = intersect_sphere(shadow_utils->shadow_ray,
				&shadow_utils->cur_s->sphere);
		if (shadow_utils->t > 0.001f && shadow_utils->t
			< shadow_utils->light_distance)
			return (1);
	...
	{
		shadow_utils->t = intersect_plane(shadow_utils->shadow_ray,
				&shadow_utils->cur_p->plane);
		if (shadow_utils->t > 0.001f && shadow_utils->t
			< shadow_utils->light_distance)
			return (1);
...
int	in_shadow(t_cor *scene, float hit_point[3], float normal[3])
{
	...
	{
		shadow_utils.t = intersect_cylinder(shadow_utils.shadow_ray,
				&shadow_utils.cur_c->cyl, &shadow_utils.dummy);
		if (shadow_utils.t > 0.001f && shadow_utils.t
			< shadow_utils.light_distance)
			return (1);
